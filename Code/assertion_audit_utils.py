...
class TestNonnegMean:
    ...
    TESTS = ['kaplan_markov','kaplan_wald','kaplan_kolmogorov','wald_sprt','kaplan_martingale','linear_hoeffding']

    @classmethod
    def empirical_Vn(sample):
        means = np.cumsum(sample) / np.arange(1, len(sample) + 1)
        lagged_means = np.concatenate(([0], means[:-1]))
        return np.cumsum((sample - lagged_means)**2)

    @classmethod
    def psi_exp(lambda_, c):
        assert 0 <= lambda_ < 1 / c
        return (-np.log(1 - c * lambda_) - c * lambda_) / c**2

    @classmethod
    def psi_exp_star(u, c):
        assert u >= 0
        return (c * u - np.log(1 + c * u)) / c**2

    @classmethod
    def psi_pois(lambda_, c):
        return (np.exp(c * lambda_) - c * lambda_ - 1) / c**2

    @classmethod
    def psi_pois_star(u, c):
        assert u >= 0
        return ((1 + c * u) * math.log(1 + c * u) - c * u) / c**2

    @classmethod
    def hks_function(g, h):
        assert np.all(g >= 0)
        assert np.all(h >= 0)
        # avoid warnings dividing by log(h / g) when we won't use it
        hh = np.where((g == 0) | (g == h), g+1, h)
        return np.where(g >= h, g * h,
                        np.where(g == 0, 0, (hh**2 - g**2) / (2 * np.log(hh / g))))

    @classmethod
    def linear_hoeffding(cls, x, N, t = 1/2, p1 = 1, random_order = True):
        """
        ...
        Uses the linear Hoeffding bound derived in Howard et al 2019 "Uniform, nonparametric, nonasymptotic confidence sequences"
        ...
        """
        #convert 0, 1/2, 1 valued samples to -1, 0, 1?
        null_mu = 2 * t - 1
        alt_mu = 2 * p1 - 1
        mean_shift = alt_mu - null_mu
        sample = 2 * np.array(x) - 1

        n = np.arange(1, len(sample) + 1)
        ballots_remaining = N + 1 - n
        Sn = np.cumsum(sample)
        lagged_Sn = np.concatenate(([0], Sn[:-1]))
        conditional_expectations = (null_mu*N - lagged_Sn) / ballots_remaining
        Mn = Sn - np.cumsum(conditional_expectations)


        Vn = np.cumsum(hks_function(conditional_expectations + 1,
                                    1 - conditional_expectations))
        #linear bound, mixture bounds would be different here
        lambda_ = mean_shift
        log_superMG = lambda_ * Mn - lambda_**2 / 2 * Vn
        superMG = np.exp(log_superMG)
        if(random_order):
            p_value = np.min(1 / superMG)
        else:
            p_value = 1 / superMG[len(x)]
        return p_value
